...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operators = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(expression, operators, env)
...
def eval_all(expressions, env):
    ...
    if not expressions:
        return None
    the_expression = expressions
    while the_expression.second is not nil:
        scheme_eval(the_expression.first, env)
        the_expression = the_expression.second
    return scheme_eval(the_expression.first, env, True)
    # END PROBLEM 8
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('{0}, {1} arguments passed in'.format(
                e,len(python_args)))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        error = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(error))
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        second = scheme_eval(first.second.first, env)
        args = Pair(second, args)
        values = Pair(first.first, values)
        bindings = bindings.second
    check_formals(values)
    return env.make_child_frame(values, args)
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        if scheme_atomp(expr):
            while (isinstance(result, Thunk)):
                result = original_scheme_eval(result.expr, expr)
        # END
